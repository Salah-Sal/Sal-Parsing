...
We introduce Recursive Language Models ( RLM s), a general-purpose inference paradigm for dramatically scaling the effective input and output lengths of LLMs. The key
...
pre\_cata = Ilm\_query(f"In Chapter 1, find... (part1). ")
...
```
L pro-     Algorithm 2 Alternate scaffold with standard (poor) design
  the user       choices for prompts, sub-calls, and code execution
  -RLM        =frac
         Input: prompt P
  the first       Output: response Y
  model  ./M
         actions <- {Finish, Exec, Search, sub_LLM,}
  prompt,      hist <- [Metadata (actions), P]      //  flaw #1
  . of it.        while True do
  and/or        |   (action, val) <- LLM.M (hist)
  that is,        if action is Finish then
         L rem return val
         values
         only ->  RUN (act.E18N,  val)
         heist <- hist || (action, val, out)
  we find       |
     t  h ist (k hist) > K them
     ising.
</REPL.
...
OOLONG (Bertsch et al., 2025). A long reasoning benchmark that requires transforming chunks of the input semantically, then aggregating these chunks to form a final answer. We report scoring based on the original paper, which scores numerical answers as score (ˆ y ) = 0 . 75 | y -ˆ y | and other answers as exact match. We focus specifically on the trec\_coarse split, a set of 50 tasks over a dataset of
...
|------------------------------------------|------------------------------------------|------------------------------------------|--------------------|--------------------|----------|-----------------|----------------------|
| Task Length N (tokens)                   | Task Length N (tokens)                   | 23K-4.2M                                 | 6M-11M             | 6M-11M             | 131K     | 131K            | 32K                  |
| GPT-5 (with RLM sub-calls to GPT-5-mini) | GPT-5 (with RLM sub-calls to GPT-5-mini) | GPT-5 (with RLM sub-calls to GPT-5-mini) |                    |                    |          |                 |                      |
| Base Model CodeAct (+                    | 24.0 ∗                                   | ($0.13 ± $0.07)                          | 0.0 ∗              | (N/A) ± (N/A)      | 44.0     | ($0.14 ± $0.02) | 0.1 ($0.16 ± $0.10)  |
| BM25)                                    | 22.0 ∗                                   | ($0.06 ± $0.08)                          | 51.0               | ($0.71 ± $1.20)    | 38.0     | ($0.61 ± $1.06) | 24.7 ($0.75 ± $0.43) |
...
| Qwen3-Coder-480B-A35B                    | Qwen3-Coder-480B-A35B                    | Qwen3-Coder-480B-A35B                    |                    |                    |          |                 |                      |
...
| Qwen3-8B                                 | Qwen3-8B                                 | Qwen3-8B                                 | Qwen3-8B           | Qwen3-8B           | Qwen3-8B | Qwen3-8B        | Qwen3-8B             |
...
return hits keywords = ["dinengdeng", "pinakbet",
...
# Create a prompt for batch processing prompt += ""numeric value', 'entity', "location', 'description and abstract cond

prompt = "Classify each of the following questions into one of these 6 categorie prompt += f"{i+1}. {question} \n"

result = 11m\_query(prompt)

return result.strip(.split("\n')

# Create the final formatted result formatted\_pairs = [f"({pair[0]}, {pair[1]})" for pair in pairs] final\_result = "\n". join(formatted\_pairs) print(f"Total pairs in final result: {len(formatted\_pairs)}")

<!-- image -->

print("First 5 pairs:")

print("\n" - join(formatted\_pairs[:5]))

FINAL\_VAR(final\_result)
